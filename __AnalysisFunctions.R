...
get_labels <- function(var_name, label_type = "value", config = NULL) {
  ...
  if (label_type == "item") {
    ...
    if (!is.null(config) && "variablen" %in% names(config)) {
      var_config <- config$variablen[config$variablen$variable_name == var_name, ]
      if (nrow(var_config) > 0 && "item_labels" %in% names(var_config)) {
        ...
        if (!is.na(item_labels_str) && item_labels_str != "") {
          ...
          if (!is.null(item_labels) && length(item_labels) > 0) {
            # Versuche direkten Match
            if (var_name %in% names(item_labels)) {
              return(item_labels[[var_name]])
            }
          }
        }
      }
...
# =============================================================================
# WRAPPER-FUNKTIONEN FÜR KOMPATIBILITÄT (nutzen zentrale get_labels)
# =============================================================================
...
# Universelle Label-Extraktion für alle Analysetypen (WRAPPER)
get_variable_label <- function(var_name, label_type = "full") {
  "WRAPPER: Nutzt zentrale get_labels() Funktion"
